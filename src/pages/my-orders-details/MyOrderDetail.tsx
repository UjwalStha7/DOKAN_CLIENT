...
<div className="py-14 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
  <div className="flex justify-start item-start space-y-2 flex-col">
    <h1 className="text-3xl dark:text-blue lg:text-4xl font-semibold leading-7 lg:leading-9 text-gray-800">Order #{orderDetails[0]?.orderId}</h1>
    <p className="text-base dark:text-gray-500 font-medium leading-6 text-gray-600">{ new Date(orderDetails[0]?.createdAt).toLocaleDateString()}</p> 
        ...
        <div className="flex justify-between xl:h-full items-stretch w-full flex-col mt-6 md:mt-0">
          ...
          <div className="flex w-full justify-center items-center md:justify-start md:items-start">
            ...
            orderDetails[0]?.Order?.orderStatus !== OrderStatus?.Cancelled && (
              <button onClick={cancelOrder} className="mt-6 md:mt-0 dark:border-white dark:hover:bg-gray-900 dark:bg-transparent dark:text-white py-5 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-gray-800 font-medium w-96 2xl:w-full text-base leading-4 text-gray-800">Cancel Order</button>
            )
           }
          </div>
        </div>
